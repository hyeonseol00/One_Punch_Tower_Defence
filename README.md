...
6. **몬스터 경로 두 갈래로 나누기**
   - currentY 값을 구할 때 inductionY 값을 230인 경우와 430인 경우 길을 만들어 path배열에 삽입합니다.
   - 해당 path 값은 User 클래스에 저장되어있고 클래스는 gameSession에 저장되어 클라이언트에 전달됩니다.
   - 클라이언트는 해당 path값을 읽고 for문으로 path 배열을 순회해 길을 그립니다.

7. **랜덤한 BGM 재생**
   - 매칭이 된 후 게임 시작 시 BGM 3곡 중 랜덤한 곡을 재생합니다.
   - 노래가 끝이나면 다음 곡으로 자동으로 재생됩니다.
   - 노래가 마지막 곡이면 처음 곡으로 돌아가 재생됩니다.
